...
import type { FilterInterface } from '~/querier/interfaces.ts';
import type { FilterDictionaryType, FilterPredicateType, FilterSelectType, FilterType } from '~/querier/types.ts';
...
export class Filter implements FilterInterface {
  ...
  public toString(search: FilterType): string {    
    ...
    ].filter(Boolean).join('')}${this.dictionary.delimiter.end}`
  }
  ...
  protected toSelectString(select?: FilterSelectType | string): string {
    ...
    for (const [key, type] of Object.entries(select)) {
      ...
      if (typeof type === 'object') {
        text += `${key}${this.dictionary.delimiter.value}${this.toString(type)}` as any
      } else {
  ...
  protected toWhereString(predicate: FilterPredicateType, isRecursion?: boolean): string {
    ...
    text += keyText ? `${this.dictionary.key.entity}${this.dictionary.delimiter.start}${keyText}${this.dictionary.delimiter.end}` : keyText
  ...
  protected toOrderString(order: { [key: string | number]: 'desc' | 'asc' }): string {
    return `${this.dictionary.key.order}${this.dictionary.delimiter.start}${Object.entries(order).map(([key, value]) => {
    ...
    }).join(this.dictionary.delimiter.array)}${this.dictionary.delimiter.end}`
  }

  protected toGroupString(group: Array<string>): string {
    return `${this.dictionary.key.group}${this.dictionary.delimiter.start}${group.join(this.dictionary.delimiter.item)}${this.dictionary.delimiter.end}`
  }
