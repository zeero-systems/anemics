...
import type { SchemaOptionsType } from '~/storer/types.ts';
import type { ColumnInterface, IndexInterface, ForeignKeyInterface, SchemaInterface, RelationInterface } from '~/storer/interfaces.ts';
...
import isForeignKey from '~/storer/guards/is-foreign-key.guard.ts';
import isIndex from '~/storer/guards/is-index.guard.ts';
import isRelation from '../guards/is-relation.guard.ts';
...
export class SchemaAnnotation implements AnnotationInterface, SchemaInterface {
  ...
  // Right now the repository on each static operation has to scrap trought
